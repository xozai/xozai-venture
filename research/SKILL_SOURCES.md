...
| [davepoon/buildwithclaude](https://github.com/davepoon/buildwithclaude/blob/main/plugins/venture-capital-intelligence/skills/financial-model/SKILL.md) (financial-model skill) | **Updated 2026-08-31, per joseleos's sign-off.** Not vendored whole (would have broken the live, CI-tested engine on UCM/Strata) — instead its DCF + revenue-multiple + SaaS-health triangulation pattern (and stage-default revenue multiples: Seed 10–15×, Series A 8–12×, Series B 5–8×) was folded into `financial-model` v3 as an additive "Triangulate" step, spec'd in the skill file. Engine implementation (Codex) and fixtures (Honey0) still pending. | MIT |
